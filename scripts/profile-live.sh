...
printf "%-6s  %8s %8s  %8s %8s  %8s %8s  %s\n" \
  "TIME" "PY_CPU%" "PY_MB" "DENO_CPU%" "DENO_MB" "CHROME%" "CHROME_MB" "LOAD"
printf "%-6s  %8s %8s  %8s %8s  %8s %8s  %s\n" \
  "------" "--------" "--------" "--------" "--------" "--------" "--------" "--------"
...
while true; do
  ...
  done < <(ps -eo %cpu,rss,command | grep -iE '[C]hrome.*Renderer|[C]hrome.*GPU|[C]hrome.*Helper' 2>/dev/null)
  ...
  printf "%-6s  %8.1f %6sMB  %8.1f %6sMB  %8.1f %6sMB  %s\n" \
    "$(date '+%H:%M:%S')" "$py_cpu" "$py_mb" "$deno_cpu" "$deno_mb" "$chrome_cpu" "$chrome_mb" "$load"
